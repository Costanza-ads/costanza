...
var __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$hooks$2f$use$2d$auth$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__ = __turbopack_context__.i("[project]/src/hooks/use-auth.ts [app-ssr] (ecmascript)"); // <-- IMPORTANTE
'use client';
;
;
;
...
function LoginPage() {
    ...
    const { login, isAuthenticated } = (0, __TURBOPACK__imported__module__$5b$project$5d2f$src$2f$hooks$2f$use$2d$auth$2e$ts__$5b$app$2d$ssr$5d$__$28$ecmascript$29$__["useAuth"])();
    ...
    if (isAuthenticated) {
        router.push('/minha-conta'); // Ou '/dashboard'
        return null;
    }
    const handleLogin = async (e)=>{
        ...
        setLoading(true);
        try {
            // 🚨 CORREÇÃO: Usamos o endpoint JWT CREATE do Djoser para Login
            const response = await fetch(`${API_URL}/api/auth/jwt/create/`, {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                // Djoser JWT CREATE espera 'email' e 'password' se configurado assim
                body: JSON.stringify({
                    email,
                    password
                })
            ...
            if (response.ok) {
                // O Djoser JWT retorna 'access' e 'refresh'. 
                const token = data.access;
                // 2. Etapa: Buscar ou Simular Dados do Usuário (Crucial para o Contexto)
                // Em um projeto real, faríamos um GET /api/auth/users/me/ usando o token.
                // Aqui, simulamos os dados necessários para o AuthContext:
                const userData = {
                    id: 1,
                    username: email,
                    email: email
                ...
                // Erro de autenticação (ex: 400 Bad Request)
    ...
    const handleSocialLogin = (provider)=>{
        ...
        window.location.href = `${API_URL}/api/auth/o/${provider}-oauth2/`;
    };
...
//# sourceMappingURL=_226a8e98._.js.map